...
double DMDens = 0; double GchibDM = 0; double MpiDM = 0; double MchiDM = 0; double LambdaDM = 0;
...
int GM2_f(const gsl_vector *x, void *params, gsl_vector *f){
	...
	double DMDens = ((struct Rparams *) params)->DMDens;
	double GchibDM = ((struct Rparams *) params)->GchibDM;
	double MpiDM = ((struct Rparams *) params)->MpiDM;
	double MchiDM = ((struct Rparams *) params)->MchiDM;
	double LambdaDM = ((struct Rparams *) params)->LambdaDM;
...
//	double PiDMField = gsl_vector_get(x,15);
//	double ChiDMField = gsl_vector_get(x,16);
...
//			", Muons: " << nMu << ", Sigmas(-) " << nSigmaminus << ", Total: " << nCharge << '\n';}
	...
	//DM self-consistency functions
//	double f15 = ChiDMField;
//	double f16 = PiDMField;
//	double f15 = pow(GchibDM,4.)*nB*nB/(pow(MpiDM*MpiDM - GchibDM*GchibDM* ChiDMField*ChiDMField, 2.)) + 0.5*MchiDM*MchiDM - LambdaDM/6.*ChiDMField*ChiDMField;
//	double f16 = 2*GchibDM*PiDMField*ChiDMField*ChiDMField - DMDens;
//	double f16 = PiDMField - GchibDM*nB/(MpiDM*MpiDM - DMDens*GchibDM/(2*PiDMField));
...
//	gsl_vector_set(f, 15, f15);
//	gsl_vector_set(f, 16, f16);
	return GSL_SUCCESS;
}
...
cout << "iter = " << iter2 << '\n' << 
	...
	<< ", Chi0: " << gsl_vector_get(S2->x, 16) << ", Pi0: " << gsl_vector_get(S2->x,15)
	<< '\n' <<
	...
	gsl_vector_get(S2->f, 13) << ", " << gsl_vector_get(S2->f, 14) << ", " << gsl_vector_get(S2->f,15) << ", " << gsl_vector_get(S2->f,16)
	<< '\n' <<
	 "============================================================" << '\n'; 
}
