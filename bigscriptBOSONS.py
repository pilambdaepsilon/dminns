...
for m in range (int(startmass), int(endmass)):
    ...
    for SIGCHIN in range (int(startsigchinpow), int(endsigchinpow)):
        ...
        for d in range (startit,endit):
            dstr = str(d)
            proc = subprocess.check_output(["./DmNoB", "%s"%dstr, "%s"%SIGCHINstr, "%s"%powstrchi2, "%s"%mstr], stderr=subprocess.STDOUT) 
        print("\nNumber Density done...\n")
        #RUN EOS FILES
        for d in range (startit,endit):
            dstr = str(d)
            proc = subprocess.check_output(["./EoSDMB", "%s"%dstr, "%s"%SIGCHINstr, "%s"%powstrchi2, "%s"%mstr], stderr=subprocess.STDOUT) 
        print("EoS done...\n")
        #RUN MR FILES
        for d in range (startit,endit):
            dstr = str(d)
            proc = subprocess.check_output(["./TOVB", "%s"%dstr, "%s"%SIGCHINstr, "%s"%powstrchi2, "%s"%mstr], stderr=subprocess.STDOUT) 
            print(d)
            dt = (time.time() - to)/60
            print("time: %f min" %(dt))
        print("MR done...\n")
